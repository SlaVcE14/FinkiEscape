...
# FinkiEscape
Апликацијата ја изработиле Славчо Јошески и Ѓорѓи Камчев
## Опис на играта
...
После завршување на дијалогот играчот има можност да притисне со глувчето на неколку означени локации кои ќе го префрлат на друга сцена.<br>
<br>
Како да знае играчот каде се тие локации ? 
- Секоја локација, кога ќе се направи `MouseHover` врз неа се менува Cursor на 
глувчето во `Cursors.Hand`, па така играчот ќе знае доколку притисне на таа локација, ќе се промени сцената за тој евент.
...
На крајот е прикажана кратка честитка и колку време било потребно за завршување на целата игра.

![FinalScene](images/FinalScene.png)

Доколку играчот реши да ја исклучи и пак да ја уклучи играта, ќе мора целиот процес повторно да го поминува.
...
```
исто така се чува и бојата со која што треба да се обојат жиците.
```c#
Color[] colors = { Color.FromArgb(224, 27, 34), Color.FromArgb(0, 114, 188), Color.FromArgb(225, 199, 26) };
```

### MouseDown
Со клик на формата се проверува дали е започнато поврзувањето на жиците (`connectingWiresStarted == true`). Ако не е, се проверува дали позицијата на глувчето е во почетната областа каде што треба да почне поврзувањето на жицата (`isStartPos(rect)`). Ако тој услов е задоволен се проверува на која област е кликнато и според тоа се одлучува која боја да биде жицата и се одредува таргет позицијата каде што треба да заврши жицата и се означива дека е започнато поврзувањето на жиците. Ако е започнато поврзувањето на жиците, се проверува дали локацијата на глувчето е во крајните области (`isEndPos(rect)`). Ако е, се проверува дали е точната крајна позиција кликната, ако и тој услов е исполнет се означува дека таа жица е поврзана и се означува дека повеќе не се поврзува таа жица.
```c#
...
### MouseMove 
во MouseMove методата се проверува дали локацијата на глувчето е во некоја од областите. Ако е, се менува курсорот на глувчето во `Cursors.Hand`.
```c#
...
```
Ако поврзувањето на жици е започнато, се црта и таа жица.
